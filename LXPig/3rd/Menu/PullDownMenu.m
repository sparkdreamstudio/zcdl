...
#define TABLEVIEW_HEIGHT [[UIScreen mainScreen]bounds].size.height
...
- (void)hideTableView:(void (^)(void))completion
{
    [UIView animateWithDuration:0.0f
                     animations:^{
                         [_currentMenuView viewWithTag:999].frame = CGRectMake(0, -TABLEVIEW_HEIGHT, [UIScreen mainScreen].bounds.size.width, TABLEVIEW_HEIGHT);
                     }
                     completion:^(BOOL finished){
                         completion();
                     }];
                        ...
                        delegate:(id<PullDownMenuDelegate>)delegate
{
    ...
    //已经显示PullDownMenu
    if (_currentMenuView == nil) {
        containerView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, 0)];
        containerView.autoresizingMask = UIViewAutoresizingFlexibleWidth;
        containerView.backgroundColor = [UIColor clearColor];//[[UIColor blackColor] colorWithAlphaComponent:0.5];
        
        _dismissButton = [UIButton buttonWithType:UIButtonTypeCustom];
        _dismissButton.frame = CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height);
        ...
        UIView *backgroundViewTemp = [[UIView alloc] initWithFrame:CGRectMake(0, -TABLEVIEW_HEIGHT, [UIScreen mainScreen].bounds.size.width, TABLEVIEW_HEIGHT)];
...
//    backgroundView.backgroundColor = _haveSub?[UIColor cloudsColor]:[UIColor whiteColor];
    if (_haveSub) {
        ...
        _subTableView = [[UITableView alloc] initWithFrame:CGRectMake(120, 0, [[UIScreen mainScreen]bounds].size.width-120, TABLEVIEW_HEIGHT) style:UITableViewStylePlain];
        _subTableView.translatesAutoresizingMaskIntoConstraints = NO;
        ...
        [containerView addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|-0-[_mainTableView]-0-[_mainTableView]-0-|" options:0 metrics:nil views:NSDictionaryOfVariableBindings(_mainTableView,_subTableView)]];
        [containerView addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"V:|-0-[_mainTableView]-0-|" options:0 metrics:nil views:NSDictionaryOfVariableBindings(_mainTableView)]];
        [containerView addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"V:|-0-[_subTableView]-0-|" options:0 metrics:nil views:NSDictionaryOfVariableBindings(_subTableView)]];
        [containerView addConstraint:[NSLayoutConstraint constraintWithItem:_mainTableView attribute:NSLayoutAttributeWidth relatedBy:NSLayoutRelationEqual toItem:_subTableView attribute:NSLayoutAttributeWidth multiplier:0.5 constant:0]];
        
    } else {
        [_mainTableView removeFromSuperview];
        _mainTableView = nil;
        _mainTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, TABLEVIEW_HEIGHT) style:UITableViewStylePlain];
        _mainTableView.autoresizingMask = UIViewAutoresizingFlexibleWidth|UIViewAutoresizingFlexibleHeight;
        _mainTableView.dataSource = self;
        _mainTableView.delegate = self;
        _mainTableView.showsHorizontalScrollIndicator = NO;
        ...
        [backgroundView addSubview:_mainTableView];
        [containerView removeConstraints:[containerView constraints]];
        [containerView addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|-0-[_mainTableView]-0-|" options:0 metrics:nil views:NSDictionaryOfVariableBindings(_mainTableView)]];
        [containerView addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"V:|-0-[_mainTableView]-0-|" options:0 metrics:nil views:NSDictionaryOfVariableBindings(_mainTableView)]];
    }
...
{
    CGRect rect = [filterView.superview convertRect:filterView.frame toView:[[UIApplication sharedApplication]keyWindow]];
    containerView.frame = CGRectMake(0, CGRectGetMaxY(rect), [[UIScreen mainScreen]bounds].size.width, 0);
}
/**
 *  选中已选择的菜单项
 */
- (void)selectRowForSelectedDetail
{
    if (_haveSub) {
        ...
        for (int i=0; i<_dataArray.count; i++)
        {
            ...
            {
                if([_selectedDetail objectForKey:@"ID"] != nil)
                {
                    if ([[childDetail objectForKey:@"ID"] isEqualToString:[_selectedDetail objectForKey:@"ID"]]) {
                        isFoundSub = YES;
                        break;
                    }
                }
        ...
        for (NSDictionary* detail in _dataArray)
        {
            if([_selectedDetail objectForKey:@"ID"] != nil)
            {
                if ([[detail objectForKey:@"ID"] isEqualToString:[_selectedDetail objectForKey:@"ID"]]) {
                    [_mainTableView selectRowAtIndexPath:[NSIndexPath indexPathForRow:i inSection:0]
                                                animated:NO scrollPosition:UITableViewScrollPositionMiddle];
                    break;
                }
